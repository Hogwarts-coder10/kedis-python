...
class UI:
    ...
    def render_config(response, sub_cmd):
        ...
        if "error" in response.lower() or "ERR" in response:
            ...
            if len(lines) >= 2:
                try:
                    # 🛡️ BULLETPROOF PARSING: Splits by the ") " prefix, strips any residual quotes
                    param = lines[0].split(") ", 1)[-1].strip('"')
    ...
    def render_table(cmd, response, raw_input):
        ...
        if cmd == "HGETALL" or (cmd == "ZRANGE" and "WITHSCORES" in raw_input.upper()):
            ...
            for i in range(0, len(lines), 2):
                try:
                    # 🛡️ BULLETPROOF PARSING: Safe extraction for hashes and leaderboards
                    field = lines[i].split(") ", 1)[-1].strip('"')
            ...
            for line in lines:
                try:
                    # 🛡️ BULLETPROOF PARSING: Safe extraction for active radar and lists
